...
public final class StudentAnalytics {
    ...
    public double averageAgeOfEnrolledStudentsParallelStream(
        ...
        double studs;
        studs =  students.parallelStream().filter(s -> s.checkIsCurrent()).mapToDouble(a -> a.getAge())
                .average().getAsDouble();
        return studs;
    ...
    public String mostCommonFirstNameOfInactiveStudentsParallelStream(
            final Student[] studentArray) {
        List<Student> inactiveStudents = new ArrayList<Student>();
        String inactiveStudentslMostCom;
        inactiveStudentslMostCom =  inactiveStudents.parallelStream()
                .filter(s -> !s.checkIsCurrent()).collect(Collectors.groupingBy(s -> s.getFirstName(), Collectors.counting()))
                .entrySet().stream().max(Map.Entry.comparingByValue()).get().getKey();
        return inactiveStudentslMostCom;
    ...
    public int countNumberOfFailedStudentsOlderThan20ParallelStream(
        ...
        List<Student> failedStudentsolderthan20 = new ArrayList<Student>();
        int res;
        failedStudentsolderthan20 =  failedStudents.parallelStream().filter(stud -> !stud.checkIsCurrent() && stud.getAge() > 20 ).toList();
        res =  (int)failedStudentsolderthan20.parallelStream().filter(stud -> !stud.checkIsCurrent() && stud.getGrade()<65).count();
        return res;
